...
import { Layout, Menu, Row, Avatar, Button, Dropdown } from "antd";
  ...
  return (
    <Layout className="!bg-[#F0F0F0] max-w-screen h-screen overflow-hidden">
      ...
      >
        ...
        >
          {menuList.map((item) => (
            <Menu.Item key={item.key} onClick={() => navigate(item.key)} className="!flex !items-center !justify-center">
              <Row className="flex items-center w-full">
                <Row className="!text-[20px]">{item.icon}</Row>
                <Row className={`${collapsed ? 'hidden' : 'ml-2'}`}>{item.label}</Row>
              </Row>
            </Menu.Item>
          ))}
        </Menu>
